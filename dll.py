...
class Node:
    ...
    def __del__(self) -> None:
        if self.has_next() and self.has_prev():
            self.prv() >> self.nxt() << self.prv()
            self.value = None
            self.remove_post_link()
            self.remove_prev_link()

        elif (not self.has_prev()) and self.has_next():
            # Node is at the head of the structure
            self.next.remove_prev_link()
            self.value = None
            self.remove_post_link()

        elif (not self.has_prev()) and (not self.has_next()):
...
class DoublyLinkedList(Iterable):
    ...
    def insertBefore(self, other: Any, node: Node) -> None:
        ...
        if node.has_prev():
            ...
            # `node` has no prior, make `other` the start of the LinkedList
            (Node(other) if type(other) is not Node else other) << node
        self._size += 1
